...
let newDeck =
    Random.self_init ();
    let rec randomDeck allCard deck = match allCard with
    | head::tail when ((Random.int 52) = 0) -> randomDeck tail (head :: deck)
    | head::tail -> randomDeck (( @ ) tail ([head])) deck
    | _ -> deck
    in
    randomDeck Card.all ([]:t)

let rec toStringListVerbose (deck:t) = match deck with
    | head::tail -> ((Card.toStringVerbose head) :: toStringListVerbose tail)
    | _ -> []

let rec toStringList (deck:t) = match deck with
    | head::tail -> ((Card.toString head) :: toStringList tail)
    | _ -> []

let drawCard = function
    | head::tail -> (head, (tail:t))
    | _ -> raise (invalid_arg "Empty Deck/!\\")
